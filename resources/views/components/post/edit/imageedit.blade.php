@props(['images' => []])
<script>
    function inputFormHandler(existImages = []){
        return{
            fields: existImages.map((img,i) => ({
                files: '',
                    id: `input-image-${i}`,
                    preview: img.url
            })),
...
<div x-data='inputFormHandler(@json(collect($images)->map(fn($image) => ["url" => asset("storage/".$image->name)])))' class="my-2">
